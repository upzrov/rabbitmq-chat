<script lang="ts">
  ...
  // Load historical messages instantly from the server load
  let allMessages = $state<(typeof archivedMessage.$inferSelect)[]>(
    data.initialMessages || [],
  );

  // DERIVED: Instantly filters the messages for the currently selected chat
  ...
  function connectWebSocket(token: string) {
    ...
    ws.onmessage = (event) => {
      ...
      if (parsed.type === "new_message") {
        ...
        // Handle unread count if the message is from someone we aren't looking at
        if (parsed.data.senderId !== targetUserId) {
          unreadCounts[parsed.data.senderId] =
            (unreadCounts[parsed.data.senderId] || 0) + 1;
        }

        ws?.send(
          JSON.stringify({ type: "ack", deliveryTag: parsed.deliveryTag }),
        );
      }
    };
  ...
  {#if !data.user}
    <div class="flex h-full items-center justify-center">
      <p class="text-gray-500">
        Please log in via better-auth to use the chat.
      </p>
    </div>
  {:else if data.availableUsers.length === 0}
    <div
      class="flex flex-col h-full items-center justify-center gap-3 p-8 text-center bg-gray-50"
    >
      <div
      ...
      <p class="text-gray-500">
        You are the only registered user on this server right now. Ask a friend
        to sign up to start chatting!
      </p>
    </div>
  {:else}
    <header class="p-5 border-b border-gray-200 bg-gray-50">
      ...
      >
        ...
        {isConnected ? "Online (Connected to RMQ)" : "Offline"}
      </div>
      ...
      <div class="mt-4 flex flex-col gap-1.5">
        ...
        >
          <option value="" disabled selected
            >Select a user to start chatting...</option
          >
    ...
    >
      ...
      {:else}
        {#each activeMessages as msg (msg.id)}
          <div
            class="p-3 rounded-2xl max-w-[80%] w-fit shadow-sm {msg.senderId ===
            data.user.id
